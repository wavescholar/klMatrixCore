...
namespace klSecurity
{
	...
	public class StreamingSecurityMessageInspector : IDispatchMessageInspector, IClientMessageInspector
	{
		...
		public void AfterReceiveReply(ref Message reply, object correlationState)
		{
			...
			var mysignedHashCharArray = mySignedHashString.ToCharArray();  
    ...
    class klCertificateManager
    {
        static void RemoveCertificates()
        {
            ...
            try
            {
                ...
                SecurityHelper.RemoveCertificates(store, "CN=klBase");
            ...
            try
            {
                ...
                SecurityHelper.RemoveCertificates(store, "CN=klClient");

                SecurityHelper.RemoveCertificates(store, "CN=klServer");
        ...
        static void ManageCertificates(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                if (args[0].Contains("-p"))
                {
                    ...
                    SecurityHelper.InstallCertificates(password);
                    return;
                }
            ...
            if (args.Length > 0)
            {
                if (args[0].Contains("-i"))
                {
                    ...
                    SecurityHelper.InstallCertificates(secureString);
                    return;
                }
